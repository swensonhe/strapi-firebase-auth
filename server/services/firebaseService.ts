...
export default ({ strapi }: Params) => ({
  ...
  overrideFirebaseAccess: async ctx => {
    ...
    let user = await strapi.plugins["users-permissions"].services.user.fetch(overrideUserId);
  ...
  checkIfUserExists: async decodedToken => {
    ...
    if (decodedToken.email) {
      user = await strapi.db.query("plugin::users-permissions.user").findOne({
        where: {
          $or: [{ appleEmail: decodedToken.email }, { email: decodedToken.email }],
        },
      });
  ...
  validateFirebaseToken: async ctx => {
    await strapi.plugin("firebase-auth").service("firebaseService").validateExchangeTokenPayload(ctx.request.body);

    const { idToken } = ctx.request.body;
    const populate = ctx.request.query.populate || [];

    const decodedToken = await strapi.plugin("firebase-auth").service("firebaseService").decodeIDToken(idToken);

    const userExists = await strapi.plugin("firebase-auth").service("firebaseService").checkIfUserExists(decodedToken);

    let user, jwt;

    if (userExists) {
      user = await strapi.plugin("firebase-auth").service("firebaseService").fetchUser(decodedToken);
    }

    if (!user) {
      // create strapi user
      user = await strapi.plugin("firebase-auth").service("firebaseService").createStrapiUser(decodedToken);
    }

    jwt = await strapi.plugin("firebase-auth").service("firebaseService").generateJWTForCurrentUser(user);

    strapi.plugin("firebase-auth").service("firebaseService").updateUserIDToken(user, idToken);
